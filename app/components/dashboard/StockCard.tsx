...
import { formatARS, formatUSD } from "@/lib/formatters/currency";
...
import { STOCK_DISPLAY_SYMBOLS, getCompanyLogoUrl } from "@/lib/constants/display";
...
export default function StockCard({ stock, currency = "ARS" }: StockCardProps) {
  const { symbol, name, price, variation, high, low, history } = stock;
  const trendPositive = variation !== null ? variation >= 0 : true;

  const formatPrice = (value: number) =>
    currency === "USD" ? formatUSD(value) : formatARS(value);

  const defaultDefinition =
    currency === "USD"
      ? "Precio de cierre diario de la acción en el mercado estadounidense (NASDAQ/NYSE), expresado en dólares (USD)."
      : "Precio de cierre diario de la acción en el mercado BYMA (Bolsas y Mercados Argentinos), expresado en pesos argentinos (ARS).";

  const displaySymbol = STOCK_DISPLAY_SYMBOLS[symbol] ?? symbol.replace(/\.[A-Z]+$/, "");
              ...
              <div className="shrink-0 flex items-center ml-0.5">
                <InfoButton title={symbol}>
                  <IndicatorDetail
                    ...
                    data={history}
                    label={symbol}
                    definition={INDICATOR_DEFINITIONS[symbol.toLowerCase()] || defaultDefinition}
      ...
      <div className="flex flex-col gap-1 mt-auto pt-2 border-t" style={{ borderColor: "var(--border-subtle)" }}>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center text-[10px] mt-1 gap-1.5 sm:gap-2">
          <div className="flex flex-col sm:flex-row gap-x-2 gap-y-0.5 min-w-0 overflow-hidden">
            <span className="truncate" style={{ color: "var(--text-muted)" }}>Min <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{formatPrice(low)}</span></span>
            <span className="truncate" style={{ color: "var(--text-muted)" }}>Max <span className="font-semibold" style={{ color: "var(--text-primary)" }}>{formatPrice(high)}</span></span>
          </div>
